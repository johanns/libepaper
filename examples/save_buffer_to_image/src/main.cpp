 ...
 * @brief Example demonstrating PNG export for debugging display layouts
 *
 * This example shows how to save framebuffer contents to PNG files
...
auto main() -> int {
  std::cout << "PNG Debug Export Example\n";
  std::cout << "========================\n\n";
  ...
  auto display_result =
      create_display<EPD27, MonoFramebuffer>(device, DisplayMode::BlackWhite, Orientation::Landscape90);
  ...
  display.draw(display.text("BMP Export Test")
                   ...
                   .build());
  display.draw(display.text("This is saved to PNG")
                   .at(10, 40)
                   .font(&Font::font16())
                   .foreground(Color::Black)
                   .background(Color::White)
  ...
  if (auto result = display.save_framebuffer_to_png("test1_text.png"); !result) {
    std::cerr << "  Failed to save PNG: " << result.error().what() << "\n";
  } else {
    std::cout << "  ✓ Saved to test1_text.png\n";
  }
  ...
  if (auto result = display.save_framebuffer_to_png("test2_shapes.png"); !result) {
    std::cerr << "  Failed to save PNG: " << result.error().what() << "\n";
  } else {
    std::cout << "  ✓ Saved to test2_shapes.png\n";
  }
  ...
  if (auto result = display.save_framebuffer_to_png("test3_fonts.png"); !result) {
    std::cerr << "  Failed to save PNG: " << result.error().what() << "\n";
  } else {
    std::cout << "  ✓ Saved to test3_fonts.png\n";
  }
  ...
  if (auto result = display.save_framebuffer_to_png("test4_numbers.png"); !result) {
    std::cerr << "  Failed to save PNG: " << result.error().what() << "\n";
  } else {
    std::cout << "  ✓ Saved to test4_numbers.png\n";
  }
  ...
  std::cout << "  display->save_framebuffer_to_png(\"debug.png\");  // Debug!\n";
