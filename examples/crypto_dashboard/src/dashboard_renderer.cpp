...
void DashboardRenderer::draw_charts(const CryptoPrice &btc, const CryptoPrice &eth, const PriceHistory &btc_history,
  ...
  // ETH chart
  display_.draw_string(135, chart_y, "ETH 30d", Font::font12(), Color::Black, Color::White);
...
void DashboardRenderer::draw_wallet_section(const CryptoPrice &btc, const CryptoPrice &eth,
  ...
  // Section title
  display_.draw_string(5, section_y, "WALLETS", Font::font12(), Color::Black, Color::White);

  // BTC balance
  std::ostringstream btc_balance;
  btc_balance.precision(4);
  btc_balance << std::fixed << "BTC: " << wallet.btc_balance; // Removed " BTC" suffix
  display_.draw_string(5, section_y + 14, btc_balance.str(), Font::font12(), Color::Black, Color::White);

  const double btc_value = wallet.btc_balance * btc.price;
  display_.draw_string(150, section_y + 14, format_price(btc_value), Font::font12(), Color::Black, Color::White);
  ...
  if (wallet.has_eth_balance()) {
    ...
    display_.draw_string(5, section_y + 28, eth_balance.str(), Font::font12(), Color::Black, Color::White);

    const double eth_value = wallet.eth_balance * eth.price;
    display_.draw_string(150, section_y + 28, format_price(eth_value), Font::font12(), Color::Black, Color::White);
  } else {
    display_.draw_string(5, section_y + 28, "ETH: API key required", Font::font12(), Color::Black, Color::White);
  }

  // Total value
  const double total_value = wallet.total_value(btc.price, eth.price);
  display_.draw_string(5, section_y + 45, "Total:", Font::font12(), Color::Black, Color::White);
  display_.draw_string(50, section_y + 43, format_price(total_value), Font::font16(), Color::Black, Color::White);
}
